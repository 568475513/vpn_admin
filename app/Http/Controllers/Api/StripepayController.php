...
class StripepayController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        \Log::info("【Stripe支付】回调接口[POST]：" . var_export($request->all(), true));

        	$result = "fail";
        	
        	$datas = $request->all();

        	if ($datas['type'] == 'source.chargeable' && $datas['data']['object']['status'] == 'chargeable') {
            	$result = "success";
                // 商户订单号
				\Stripe\Stripe::setApiKey(self::$systemConfig['stripepay_api_key']);
        		\Stripe\Charge::create(['amount' => $datas['data']['object']['amount'], 'currency' => 'HKD', 'source' => $datas['data']['object']['id']]);
				exit($result);
            }

            if ($datas['type'] == 'charge.succeeded' && $datas['data']['object']['status'] == 'succeeded') {
            	$result = "success";
                // 商户订单号
                $tradedata = [];
                $tradedata['sn'] = $datas['data']['object']['source']['id'];
                $this->tradePaid($tradedata);
                 // 返回验证结果
        		exit($result);
            } else {
                Log::info('Stripe支付-POST:交易失败[' . getClientIp() . ']');
            }
    ...
    {
        Log::info('【Stripe支付】回调交易支付');

        // 获取未完成状态的订单防止重复增加时间
        $payment = Payment::query()->with(['order', 'order.goods'])->where('status', 0)->where('sn', $msg['sn'])->first();
        if (!$payment) {
            Log::info('【Stripe支付】回调订单不存在');
            return;
        }
        ...
        try {
           ...
           //$payment->pay_way = 2; // 1-微信、2-支付宝
            ...
            DB::rollBack();
            Log::info('【Stripe支付】回调更新支付单和订单异常：' . $e->getMessage());
        }
    }
