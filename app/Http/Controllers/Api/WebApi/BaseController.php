...
class BaseController
{
    ...
    public function setNodeStatus(Request $request, Node $node): JsonResponse
    {
        ...
        $data = array_map('intval', $validator->validate());
    ...
    public function setNodeOnline(Request $request, Node $node): JsonResponse
    {
        ...
        $onlineCount = 0;
        foreach ($validator->validate() as $input) { // 处理节点在线IP数据
    ...
    public function setUserTraffic(Request $request, Node $node): JsonResponse
    {
        ...
        foreach ($validator->validate() as $input) { // 处理用户流量数据
    ...
    // 获取节点的审计规则
    public function getNodeRule(Request $request,Node $node): JsonResponse
    {
        // 节点未设置任何审计规则

        if ($ruleGroup = $node->ruleGroupNode->ruleGroup) {
           
           $ids = explode(",", $ruleGroup->rules);
            $rules = Rule::whereIn('id', $ids)->get();
