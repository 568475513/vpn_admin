...
class AdminController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $view['onlineUserCount'] = User::query()->where('t', '>=', time() - 300)->count(); // 10分钟内在线用户数
        ...
        if (!$userTotalTrafficList->isEmpty()) {
            foreach ($userTotalTrafficList as $vo) {
                if ($vo->totalTraffic > (self::$systemConfig['traffic_ban_value'] * 1024 * 1024)) {
    ...
    public function userList(Request $request)
    {
        ...
        foreach ($userList as &$user) {
            ...
            $user->trafficWarning = $totalTraffic > (self::$systemConfig['traffic_ban_value'] * 1024 * 1024) ? 1 : 0;
    ...
    public function nodeList(Request $request)
    {
        ...
        foreach ($nodeList as &$node) {
            ...
            // 负载（10分钟以内）
            $node_info = SsNodeInfo::query()->where('node_id', $node->id)->where('log_time', '>=', strtotime("-5 minutes"))->orderBy('id', 'desc')->first();
    ...
    public function addNode(Request $request)
    {
        if ($request->isMethod('POST')) {
            ...
            try {
                ...
                $ssNode->port = intval($request->get('port'));
                $ssNode->speed_limit = intval($request->get('speed_limit'));
                $ssNode->client_limit = intval($request->get('client_limit'));
                $ssNode->save();
        ...
        if ($request->isMethod('POST')) {
            ...
            try {
                $data = [
                    ...
                    'port'           => intval($request->get('port')),
                    'speed_limit'  => intval($request->get('speed_limit')),
                    'client_limit' => intval($request->get('client_limit'))
                ];
    ...
    public function analysis(Request $request)
    {
        ...
        if (false === $logs) {
            ...
            foreach ($logs as $log) {
                ...
                preg_match('/(.*)at time (.*)/', $log, $log_time);
                if (!empty($log_time)) {
                   $html = '['.$log_time[2].'] - ';
                } else {
 					$html = '';
               ...
               preg_match('/TCP request (.*) by user/', $log, $tcp_matches);
               if (!empty($tcp_matches)) {
                 $html .= ' [TCP] '.$tcp_matches[1];
               } else {
                   preg_match('/UDP data to (.*) from/', $log, $udp_matches);
                 	if (!empty($udp_matches)) {
                       $html .= ' [UDP] '.$udp_matches[1];
                    }  
               }
              
                preg_match('/(.*)by user (.*) at/', $log, $log_user);
                 if (!empty($log_user)) {
                  $html .= ' - [USER] '.$log_user[2];
                } 
 ...
 //var_dump($view);exit;
    ...
    public function setConfig(Request $request)
    {
        ...
        if (in_array($name, ['is_youzan'])) {
            ...
            $is_stripepay = Config::query()->where('name', 'is_stripepay')->first();
            if ($is_stripepay->value) {
                return Response::json(['status' => 'fail', 'data' => '', 'message' => '已经在使用【Stripe支付】']);
            }
        }
        ...
        if (in_array($name, ['is_stripepay'])) {
            ...
            $is_wechatpay = Config::query()->where('name', 'is_wechatpay')->first();
            if ($is_wechatpay->value) {
                return Response::json(['status' => 'fail', 'data' => '', 'message' => '已经在使用【微信支付】']);
            }
        }
        
        if (in_array($name, ['is_wechatpay'])) {
            $is_youzan = Config::query()->where('name', 'is_youzan')->first();
            if ($is_youzan->value) {
                return Response::json(['status' => 'fail', 'data' => '', 'message' => '已经在使用【有赞云支付】']);
            }
    ...
    public function orderList(Request $request)
    {
		...
		$query_total->whereIn('status',[1,2]);
		
        if ($range_time) {
            $range_time = explode('至', $range_time);
            $query->where('created_at', '>=', trim($range_time['0']))->where('created_at', '<=', trim($range_time[1]));
            $query_total->where('created_at', '>=', trim($range_time['0']))->where('created_at', '<=', trim($range_time[1]));
        }
        ...
        $view['total_price'] = $query_total->sum('amount');
    ...
    public function onlineIPMonitor(Request $request)
    {
        ...
        $query = SsNodeIp::query()->with(['node', 'user'])->where('type', 'tcp')->where('created_at', '>=', strtotime("-60 seconds"));
  ...
  /**
    ...
    public function gameList(Request $request)
    {
        ...
        return Response::view('admin.gameList', $view);
    }
    ...
    public function addGame(Request $request)
    {
        if ($request->isMethod('POST')) {
            ...
            if ($request->hasFile('logo')) {
                ...
                // 验证文件合法性
                if (!in_array($fileType, ['jpg', 'png', 'jpeg', 'bmp','ico'])) {
                    Session::flash('errorMsg', 'LOGO不合法');
            ...
            $game->platform = $request->get('platform', 'pc');
            $game->type = $request->get('type', 1);
            $game->author = $request->get('author');
            $game->summary = $request->get('summary');
            $game->logo = $logo;
            $game->content = $request->get('editorValue');
            $game->sort = $request->get('sort', 0);
            $game->save();
            ...
            return Response::view('admin.addGame');
        }
    }
    ...
    public function editGame(Request $request)
    {
        ...
        if ($request->isMethod('POST')) {
            ...
            $ret = Game::query()->where('id', $id)->update($data);
            if ($ret) {
                Session::flash('successMsg', '编辑成功');
            } else {
                Session::flash('errorMsg', '编辑失败');
            ...
            return Redirect::to('admin/editGame?id=' . $id);
        } else {
            $view['game'] = Game::query()->where('id', $id)->first();

            return Response::view('admin.editGame', $view);
        }
    }
    ...
    public function delGame(Request $request)
    {
        ...
        $ret = Game::query()->where('id', $id)->delete();
        if ($ret) {
            return Response::json(['status' => 'success', 'data' => '', 'message' => '删除成功']);
        } else {
            return Response::json(['status' => 'fail', 'data' => '', 'message' => '删除失败']);
