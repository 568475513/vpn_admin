    ...
    <form class="register-form" id="register-form" action="{{url('register')}}" method="post" style="display: block;">
        @if(\App\Components\Helpers::systemConfig()['is_register'])
            ...
            <div class="form-group">
                ...
                <input class="form-control placeholder-no-fix" type="text" autocomplete="off" placeholder="请输入手机号码" name="username" id="username" value="{{Request::old('username')}}" required />
            ...
            <div class="form-group" style="margin-bottom:75px;">
                    ...
                    <input class="form-control placeholder-no-fix" style="width:60%;float:left;" type="text" autocomplete="off" placeholder="验证码" name="code" value="{{Request::old('code')}}" required />
            ...
            @if(\App\Components\Helpers::systemConfig()['is_invite_register'])
                <div class="form-group">
                    ...
                    <input class="form-control placeholder-no-fix" type="text" autocomplete="off" placeholder="{{trans('register.code')}}" name="invite_code" value="{{Request::old('invite_code') ? Request::old('invite_code') : Request::get('invite_code')}}" @if(\App\Components\Helpers::systemConfig()['is_invite_register'] == 2) required @endif />
                </div>
            ...
            $.ajax({
                ...
                success: function (ret) {
                    if (ret.status == 'fail') {
                        ...
                        layer.alert('验证码已发送至您的手机，请稍作等待', {icon:1, title:'提示'});
